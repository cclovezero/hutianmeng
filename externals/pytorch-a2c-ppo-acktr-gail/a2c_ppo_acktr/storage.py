...
class RolloutStorage(object):
    ...
    def update_utopian_z(self):
        # update utopian and nadir
        self.utopian_z = self.returns[:-1].max(axis = 0).values.max(axis = 0).values
        self.utopian_z = torch.max(self.utopian_z, self.value_preds[:-1].max(axis = 0).values.max(axis = 0).values)
        self.nadir_z = self.returns[:-1].min(axis = 0).values.min(axis = 0).values
        self.nadir_z = torch.min(self.nadir_z, self.value_preds[:-1].min(axis = 0).values.min(axis = 0).values)
    ...
    def feed_forward_generator(self,
        ...
        for indices in sampler:
            ...
            if advantages is None:
                ...
                # adv_targ = advantages.view(-1, 1)[indices]  # 有修改！
                adv_targ = advantages.view(-1, 2)[indices]
